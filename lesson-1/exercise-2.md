...
Step 1:
Define a TensorFlow constant and store the integer 2 in it (documentation). Assign it to the Python variable ‘x’.
...
Step 6:
The * operator is translated into the TensorFlow multiply operation. Perform the same multiplication as you did in Step 3, but do so using the multiply operation explicitly (documentation). Store the operation in a Python variable called ‘tf_z’.
